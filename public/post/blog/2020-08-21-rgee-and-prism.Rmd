...
Now we can extract the **sum** of the watershed per month per year. The sum function is basically taking each pixel within this area per month per year and summing the 'ppt' values, which will be a lot of mm's gettin added together.
...
prism_processed <- prism_tabacco %>%
  pivot_longer(-name, names_to = "Date", values_to = "precip") %>%
...
prism_processed %>%  ggplot(aes(month, precip)) + geom_boxplot(aes(group = month)) + 
...
```{r, echo=FALSE}
prism_processed %>% ggplot(aes(month, precip)) + geom_line(aes(color = year, group = year)) + scale_color_distiller(palette = "Spectral")+ theme_classic()  + scale_x_continuous(breaks = seq(1,12,1)) + labs(x = "Months", y = "Total Monthly Precipitation (mm)", title = "Looking at monthly total precipitation per year per month.") + scale_y_continuous(labels = scales::comma_format())
```

These plots are actually not that bad if you can make them interactive by using `plotly`. The graph below will let you hover over areas and get data depending on what your eye hones in on. 

```{r, echo = FALSE, warning=FALSE, message=FALSE}
p1 <- prism_processed %>% ggplot(aes(month_day, precip)) + geom_line(aes(color = decade, group = decade)) + scale_color_distiller(palette = "RdBu")+ theme_classic() + theme(axis.ticks = element_blank(), axis.text.x = element_blank()) + labs(x = "Years-Months-Days within 10 year partitions, e.g. 1980-1989, etc.", y = "Total Precipitation (mm)", title = "Looking at decades of annual total precipitation.") + scale_y_continuous(labels = scales::comma_format())
...
So a way to do this is to look at the graphs in a normalized way or to just plot them on top of each other. 

```{r message=FALSE}

river_total <- tobacco_river %>% filter(!year %in% c("1958", "1959", "2019" , "2020")) %>% group_by(year, month) %>% summarise(total_q = sum(Flow))
...
```{r}
p2 <- all_tobacco %>% filter(decade == "2010" ) %>% mutate(across(matches(c("precip", "total_q")), scale)) %>% pivot_longer(cols = c("precip", "total_q"), names_to = "water", values_to = "total_water") %>% ggplot(aes(month, total_water)) + geom_line(aes(color = water)) + facet_wrap(~year, scales = "free_y") + scale_x_continuous(breaks = seq(1,12, 1)) + theme_light()

plotly::ggplotly(p2)
```

This is pretty cool. There are some great eye-ball test relationships between the two. However, we are just exploring and this would need a proper time-series analysis to see what kind of suprise we could generate. For now though, let's just end there. We'll have some good data sets to continue on with the distribution exploration later. Have a good one!
