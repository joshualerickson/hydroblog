...
library(DT)
all_tobacco_DT <- as_tibble(all_tobacco) %>% select(Drainage = "name", 'Precipitation (mm)' = "precip", Year = "year", Month = "month", 'Total Discharge (cfs)' = "total_q", 'Max Discharge (cfs)' = "max_q", 'SD Discharge (cfs)'  = "sd_q") %>% mutate(`Precipitation (mm)` = round(`Precipitation (mm)`,0))
```
